...
Branch for Excel sheet **Security URL (API Service)** from `Testable_Strategy_Metrics_Mapping_v0.2`.

Python primary tool versions reference: `Final_Enterprise_Mapping_Matrix 1.xlsx` (Python sheet).

**13 L3 techniques**, **17 L4/L5 mappings**.
...
- **Broken Access Control Testing** → Unauthorized Access Count (excel tool: `OWASP ZAP`, python: `pylint` / `Ruff`)
...
- **Change Control Verification** → Unreviewed Change Count (excel tool: `GitHub API (branch protection)`, python: `pylint` / `Ruff`)
...
- **OWASP Top 10 Vulnerability Scan** → OWASP High/Critical Finding Count (excel tool: `OWASP ZAP`, python: `pylint` / `Ruff`)
- **SQL Injection Testing** → SQLi Vulnerability Count (excel tool: `OWASP ZAP`, python: `pylint` / `Ruff`)
- **XSS Testing** → XSS Vulnerability Count (excel tool: `OWASP ZAP`, python: `pylint` / `Ruff`)
- **Server-Side Request Forgery (SSRF) Testing** → SSRF Vulnerability Count (excel tool: `OWASP ZAP`, python: `pylint` / `Ruff`)
